...
export const metadata = { title: "Nouvelle pièce récupérée" };

export default async function NewRecoveredPartPage() {
  const { t } = await getAppI18n();
  const session = await getSession();
  if (!session) redirect("/login");
  if (!hasPermission(session.role, "inventory:manage")) redirect("/dashboard/inventory/recovered-parts");

  const storeId = session.storeIds[0];
  if (!storeId) redirect("/dashboard/inventory/recovered-parts");
  ...
  return (
    <>
      <PageHeader
        title={t("inventory.newRecoveredPart")}
        description={t("inventory.recoveredPartsDescription")}
      />
