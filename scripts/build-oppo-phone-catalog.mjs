...
const sourcePath = path.join(root, "src/data/catalog/phones/prompts/phones_oppo_full_public_phone_catalog_v1.json");
const brandPath = path.join(root, "src/data/catalog/phones/brands/oppo.json");
const tsPath = path.join(root, "prisma/catalog/oppo-catalog-enriched.ts");
const reportPath = "/private/tmp/oppo-phone-catalog-build-report.json";
...
function isOppoPhoneName(name) {
  if (!/^oppo\b/i.test(name)) return false;
...
function recordFromAllPhones(row) {
  ...
  if (!isOppoPhoneName(name)) return null;
...
function recordFromProcessed(row) {
  ...
  if (clean(row.phone_brand).toLowerCase() !== "oppo" || !isOppoPhoneName(name)) return null;
  return {
    name,
...
function recordFromLegacy(row) {
  if (clean(row.brand).toLowerCase() !== "oppo") return null;
  const name = `OPPO ${clean(row.model)}`;
  if (!isOppoPhoneName(name)) return null;
  return {
    name,
    releaseYear: clean(row.status).match(/\b(20\d{2}|19\d{2})\b/)?.[1] ? Number(clean(row.status).match(/\b(20\d{2}|19\d{2})\b/)[1]) : null,
...
function imageCandidateSlugs(modelName) {
  ...
  const noBrand = slugify(modelName.replace(/^oppo\s+/i, ""));
  const set = new Set();
  const baseItems = [
    base,
    noBrand,
    noBrand ? `oppo-${noBrand}` : "",
  ];
  for (const item of baseItems) {
    if (!item) continue;
    for (const suffix of ["", "-", "--", "-1", "-2", "-new", "-5g", "-4g"]) set.add(`${item}${suffix}`);
    set.add(item.replace(/find-x-(\d+)/, "find-x$1"));
    set.add(item.replace(/reno-(\d+)/, "reno$1"));
    set.add(item.replace(/a-(\d+)/, "a$1"));
    set.add(item.replace(/f-(\d+)/, "f$1"));
    set.add(item.replace(/k-(\d+)/, "k$1"));
  }
...
function pageCandidateSlugs(modelName) {
  return [...new Set([
    slugify(modelName),
    slugify(modelName.replace(/^oppo\s+/i, "")),
...
const sourceBrand = source.brands.find((brand) => clean(brand.name).toLowerCase() === "oppo");
if (!sourceBrand) throw new Error("Oppo brand not found");
...
const brand = {
  ...
  sortOrder: 6,
  aliases: ["OPPO"],
  logoUrl: sourceBrand.logo_url ?? "https://logo.clearbit.com/oppo.com",
...
for (const family of sourceBrand.families) {
  for (const sourceModel of family.models || []) {
    ...
    const model = {
      ...
      sources: [...new Set([...(sourceModel.sources || []), ...(record ? ["kaggle:gsmarena-derived-oppo-specs"] : [])])],
    };
