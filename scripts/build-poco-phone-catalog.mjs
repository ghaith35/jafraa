...
const sourcePath = path.join(root, "src/data/catalog/phones/prompts/phones_poco_full_public_phone_catalog_v1.json");
const brandPath = path.join(root, "src/data/catalog/phones/brands/poco.json");
const tsPath = path.join(root, "prisma/catalog/poco-catalog-enriched.ts");
const reportPath = "/private/tmp/poco-phone-catalog-build-report.json";
...
function isPocoPhoneName(name) {
  if (!/^(poco|xiaomi poco)\b/i.test(name)) return false;
  if (/\b(?:pad|tablet|watch|band)\b/i.test(name)) return false;
  return true;
}

function recordFromAllPhones(row) {
  const name = clean(row.Name);
  if (!isPocoPhoneName(name)) return null;
...
function recordFromProcessed(row) {
  ...
  if (clean(row.phone_brand).toLowerCase() !== "xiaomi" || !isPocoPhoneName(name)) return null;
  return {
    name,
...
function imageCandidateSlugs(modelName) {
  ...
  const noBrand = slugify(modelName.replace(/^xiaomi\s+/i, "").replace(/^poco\s+/i, ""));
  const set = new Set();
  const baseItems = [
    base,
    noBrand,
    base.startsWith("poco-") ? `xiaomi-${base}` : "",
    noBrand ? `poco-${noBrand}` : "",
    noBrand ? `xiaomi-poco-${noBrand}` : "",
  ];
  for (const item of baseItems) {
    if (!item) continue;
    for (const suffix of ["", "-", "--", "-1", "-2", "-new", "-5g", "-4g"]) set.add(`${item}${suffix}`);
    set.add(item.replace(/^xiaomi-poco-/, "poco-"));
    set.add(item.replace(/^poco-/, "xiaomi-poco-"));
    set.add(item.replace(/note-(\d+)/, "note$1"));
  }
...
function pageCandidateSlugs(modelName) {
  return [...new Set([
    slugify(modelName),
    slugify(modelName.replace(/^xiaomi\s+/i, "").replace(/^poco\s+/i, "")),
...
const sourceBrand = source.brands.find((brand) => clean(brand.name).toLowerCase() === "poco");
if (!sourceBrand) throw new Error("Poco brand not found");

const { records, byName, byLooseName } = loadDatasetRecords();
const brand = {
  name: "Poco",
  sortOrder: 5,
  aliases: ["POCO", "Xiaomi POCO"],
  logoUrl: sourceBrand.logo_url ?? "https://logo.clearbit.com/po.co",
...
for (const family of sourceBrand.families) {
  for (const sourceModel of family.models || []) {
    ...
    const model = {
      ...
      sources: [...new Set([...(sourceModel.sources || []), ...(record ? ["kaggle:gsmarena-derived-poco-specs"] : [])])],
    };
