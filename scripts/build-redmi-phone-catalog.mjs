...
const sourcePath = path.join(root, "src/data/catalog/phones/prompts/phones_redmi_full_public_phone_catalog_v2_reconciled.json");
const brandPath = path.join(root, "src/data/catalog/phones/brands/redmi.json");
const tsPath = path.join(root, "prisma/catalog/redmi-catalog-enriched.ts");
const reportPath = "/private/tmp/redmi-phone-catalog-build-report.json";
...
function isRedmiPhoneName(name) {
  if (!/^(redmi|xiaomi redmi)\b/i.test(name)) return false;
...
function recordFromAllPhones(row) {
  ...
  if (!isRedmiPhoneName(name)) return null;
...
function recordFromProcessed(row) {
  ...
  if (clean(row.phone_brand).toLowerCase() !== "xiaomi" || !isRedmiPhoneName(name)) return null;
  return {
    name,
...
function imageCandidateSlugs(modelName) {
  ...
  const noBrand = slugify(modelName.replace(/^xiaomi\s+/i, "").replace(/^redmi\s+/i, ""));
  const set = new Set();
  const baseItems = [
    base,
    noBrand,
    base.startsWith("redmi-") ? `xiaomi-${base}` : "",
    noBrand ? `redmi-${noBrand}` : "",
    noBrand ? `xiaomi-redmi-${noBrand}` : "",
  ];
  for (const item of baseItems) {
    if (!item) continue;
    for (const suffix of ["", "-", "--", "-1", "-2", "-new", "-5g", "-4g"]) set.add(`${item}${suffix}`);
    set.add(item.replace(/^xiaomi-redmi-/, "redmi-"));
    set.add(item.replace(/^redmi-/, "xiaomi-redmi-"));
...
function pageCandidateSlugs(modelName) {
  return [...new Set([
    slugify(modelName),
    slugify(modelName.replace(/^xiaomi\s+/i, "").replace(/^redmi\s+/i, "")),
...
const sourceBrand = source.brands.find((brand) => clean(brand.name).toLowerCase() === "redmi");
if (!sourceBrand) throw new Error("Redmi brand not found");
...
for (const family of sourceBrand.families) {
  for (const sourceModel of family.models || []) {
    ...
    const model = {
      ...
      sources: [...new Set([...(sourceModel.sources || []), ...(record ? ["kaggle:gsmarena-derived-redmi-specs"] : [])])],
    };
