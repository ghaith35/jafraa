...
const sourcePath = path.join(root, "src/data/catalog/phones/prompts/phones_condor_full_public_phone_catalog_v1.json");
const brandPath = path.join(root, "src/data/catalog/phones/brands/condor.json");
const tsPath = path.join(root, "prisma/catalog/condor-catalog-enriched.ts");
const reportPath = "/private/tmp/condor-phone-catalog-build-report.json";
...
const IMAGE_URL_OVERRIDES = new Map(Object.entries({
  "Condor Nova 80 Pro": "https://packpromodzz.com/cdn/shop/files/8bd5e9c1-e2a1-473f-8f17-b02e23dd40f4.jpg?v=1771356283",
}));
...
function isCondorPhoneName(name) {
  if (!/^condor\b/i.test(name)) return false;
...
function recordFromAllPhones(row) {
  ...
  if (!isCondorPhoneName(name)) return null;
...
function recordFromProcessed(row) {
  ...
  if (clean(row.phone_brand).toLowerCase() !== "condor" || !isCondorPhoneName(name)) return null;
  return {
    name,
...
function recordFromLegacy(row) {
  if (clean(row.brand).toLowerCase() !== "condor") return null;
  const name = `CONDOR ${clean(row.model)}`;
  if (!isCondorPhoneName(name)) return null;
  return {
    name,
    releaseYear: clean(row.status).match(/\b(20\d{2}|19\d{2})\b/)?.[1] ? Number(clean(row.status).match(/\b(20\d{2}|19\d{2})\b/)[1]) : null,
...
function imageCandidateSlugs(modelName) {
  ...
  const noBrand = slugify(modelName.replace(/^condor\s+/i, ""));
  const set = new Set();
  const baseItems = [
    base,
    noBrand,
    noBrand ? `condor-${noBrand}` : "",
  ];
  for (const item of baseItems) {
    if (!item) continue;
    for (const suffix of ["", "-", "--", "-1", "-2", "-new", "-5g", "-4g"]) set.add(`${item}${suffix}`);
    set.add(item.replace(/f-(\d+)/, "f$1"));
    set.add(item.replace(/nova-(\d+)/, "nova$1"));
    set.add(item.replace(/griffe-(\w+)-(\d+)/, "griffe-$1$2"));
    set.add(item.replace(/plume-(\w+)-(\d+)/, "plume-$1$2"));
    set.add(item.replace(/allure-(\w+)-(\d+)/, "allure-$1$2"));
  }
...
function pageCandidateSlugs(modelName) {
  return [...new Set([
    slugify(modelName),
    slugify(modelName.replace(/^condor\s+/i, "")),
...
async function findImage(modelName, preferred = null) {
  ...
  const mobileSpecsUrl = `https://mobilespecs.net/image/mobile/${modelName.toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_|_$/g, "")}.jpg`;
  if (await isValidImage(mobileSpecsUrl)) return { imageUrl: mobileSpecsUrl, source: "mobilespecs:condor-product-image" };
...
const sourceBrand = source.brands.find((brand) => clean(brand.name).toLowerCase() === "condor");
if (!sourceBrand) throw new Error("Condor brand not found");
...
const brand = {
  ...
  sortOrder: 21,
  aliases: ["CONDOR"],
  logoUrl: sourceBrand.logo_url ?? "https://logo.clearbit.com/condor.dz",
...
for (const family of sourceBrand.families) {
  for (const sourceModel of family.models || []) {
    ...
    const specs = {
      ...
      processor: clean(record?.specs.processor) || clean(sourceSpecs.processor) || (clean(sourceSpecs.ram).toUpperCase().endsWith("MB") ? "Feature phone chipset" : "Entry-level mobile chipset"),
    ...
    const model = {
      ...
      sources: [...new Set([...(sourceModel.sources || []), ...(record ? ["kaggle:gsmarena-derived-condor-specs"] : [])])],
    };
