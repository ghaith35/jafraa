...
const sourcePath = path.join(root, "src/data/catalog/phones/prompts/phones_lg_full_public_phone_catalog_v1.json");
const brandPath = path.join(root, "src/data/catalog/phones/brands/lg.json");
const tsPath = path.join(root, "prisma/catalog/lg-catalog-enriched.ts");
const reportPath = "/private/tmp/lg-phone-catalog-build-report.json";
...
function isLGPhoneName(name) {
  if (!/^lg\b/i.test(name)) return false;
...
function recordFromAllPhones(row) {
  ...
  if (!isLGPhoneName(name)) return null;
...
function recordFromProcessed(row) {
  ...
  if (clean(row.phone_brand).toLowerCase() !== "lg" || !isLGPhoneName(name)) return null;
  return {
    name,
...
function recordFromLegacy(row) {
  if (clean(row.brand).toLowerCase() !== "lg") return null;
  const name = `LG ${clean(row.model)}`;
  if (!isLGPhoneName(name)) return null;
  return {
    name,
    releaseYear: clean(row.status).match(/\b(20\d{2}|19\d{2})\b/)?.[1] ? Number(clean(row.status).match(/\b(20\d{2}|19\d{2})\b/)[1]) : null,
...
function imageCandidateSlugs(modelName) {
  ...
  const noBrand = slugify(modelName.replace(/^lg\s+/i, ""));
  const set = new Set();
  const baseItems = [
    base,
    noBrand,
  ...
  for (const item of baseItems) {
    ...
    set.add(item.replace(/v-(\d+)/, "v$1"));
    set.add(item.replace(/k-(\d+)/, "k$1"));
    set.add(item.replace(/q-(\d+)/, "q$1"));
    set.add(item.replace(/x-(\w+)/, "x-$1"));
...
function pageCandidateSlugs(modelName) {
  return [...new Set([
    slugify(modelName),
    slugify(modelName.replace(/^lg\s+/i, "")),
...
const sourceBrand = source.brands.find((brand) => clean(brand.name).toLowerCase() === "lg");
if (!sourceBrand) throw new Error("LG brand not found");
...
const brand = {
  ...
  sortOrder: 19,
  aliases: ["LG"],
  logoUrl: sourceBrand.logo_url ?? "https://logo.clearbit.com/lg.com",
...
for (const family of sourceBrand.families) {
  for (const sourceModel of family.models || []) {
    ...
    const model = {
      ...
      sources: [...new Set([...(sourceModel.sources || []), ...(record ? ["kaggle:gsmarena-derived-lg-specs"] : [])])],
    };
